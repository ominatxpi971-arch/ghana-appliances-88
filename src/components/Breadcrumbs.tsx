...
export default function Breadcrumbs({ items }: { items: Crumb[] }) {
  return (
    <nav aria-label="Breadcrumb" itemScope itemType="https://schema.org/BreadcrumbList">
      <ol className="flex items-center gap-1 text-sm text-gray-400 mb-4 overflow-x-auto whitespace-nowrap list-none p-0 m-0">
        ...
        {items.map((item, i) => (
          <li key={i} itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem" className="flex items-center gap-1">
            ...
            {item.href ? (
              ...
              <span itemProp="name" className="text-gray-600 font-medium">{item.label}</span>
            )}
